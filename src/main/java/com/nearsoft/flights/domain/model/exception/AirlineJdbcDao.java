...
public class AirlineJdbcDao implements AirlineDao {
	...
	public Airline findByAirlineCode(Connection conn, String airlineCode) {
		...
		try {
			...
			logger.error(ex);
			throw new RepositoryException("Error occured while fetching airline data", ex);
		} finally {
	...
	private void insert(Connection conn, Airline airline) {
		...
		try {
			...
			ex.printStackTrace();
			logger.error(ex);
			throw new RepositoryException("Error ocurred while inserting airline data", ex);
		} finally {
			JdbcUtils.close(st);
		}
	}
	...
	@Override
	public void deleteAll(Connection conn) throws RepositoryException {
